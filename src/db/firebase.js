...
export const images = firebase
  .storage()
  .ref()
  .child("images"); // Storage
...
export const getMovieCover = async snapshot => {
  ...
  for (let i = 0; i < moviesWithoutCover.length; i++) {
    try {
      ...
      let coverURL = await images
        .child(`${moviesWithoutCover[i].title.toLowerCase()}.jpg`)
        .getDownloadURL();
      movieWithCover.push({
...
// Getting image cover for movies or TV Shows
export const getMovieCoverNonDB = async movies => {
  let moviesWithoutCover = movies;
  let movieWithCover = [];
